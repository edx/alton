...
def warn_direct_message(func):
    """
    Decorator which disallows messaging back to a user in a room.
    """
    def check_direct_message(plugin, message, *args, **kwargs):
        """
        If message is to room, reply differently.
        """
...
class UserPlugin(WillPlugin):
    ...
    @respond_to(r"^twofactor verify (?P<token>\w+)")
    @warn_direct_message
    ...
    @respond_to(r"^twofactor remove (?P<nick>\w+)")
    ...
    @respond_to(r"^(?:show permissions for|what can) (?P<nick>\w+)(?: do|)")
    ...
    @respond_to(r"^who can (?P<permission>([\w.:-]+))")
    ...
    @respond_to(r"^can I(?P<permissions>( [\w.:-]+)+)")
    ...
    @respond_to(r"^(?:give|grant) (?P<nick>\w+) permission(?P<permissions>( [\w.:-]+)+)")
    ...
    @respond_to(r"^(?:revoke|take away) (?P<permissions>(\w+ )+)from (?P<nick>\w+)")
