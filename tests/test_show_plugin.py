...
from plugins.show import Versions, ShowPlugin
...
class TestParseCutAmi(unittest.TestCase):
    """
    Test the parsing of commands for cutting an AMI.
    """
    valid = [
    ...
    def test_valid(self):
        for test in self.valid:
            try:
                ShowPlugin._parse_cut_ami(test['text'])  # pylint: disable=protected-access
            except ParseException as exc:
                self.fail('Parsing failed for string: `{}` error: `{}` message: `{}`'.format(
                    test['text'], exc.message, test['msg']))
    ...
    def test_basic_properties(self):
        ...
        result = ShowPlugin._parse_cut_ami(text)  # pylint: disable=protected-access
...
class TestCutFromEdp(unittest.TestCase):
    """
    Tests for cutting AMIs from EDP.
    """
    ...
    def test_complex_result(self, mocked_notify_abbey, *args):  # pylint: disable=unused-argument
        message = mock.Mock()
    ...
    @mock.patch.object(ShowPlugin, '_notify_abbey')     # this is how we test the result
    def test_basic_result(self, mocked_notify_abbey, mocked_get_ami_versions, *args):  # pylint: disable=unused-argument
        message = mock.Mock()
        show_plugin = ShowPlugin()
        ...
        # spec: self._notify_abbey(message, dest_env, dest_dep, dest_play,
        #    final_versions, noop, dest_running_ami, verbose)
        mocked_notify_abbey.assert_called_with(
            message, 'foo', 'bar', 'baz',
